...
def esm2_checkpoint_directory() -> Path:
    ...
    return (
        Path(configured)
        if configured
        else Path.home() / ".cache" / "torch" / "hub" / "checkpoints"
    )


def esm2_representation_contract() -> Dict[str, object]:
    """Return the exact representation contract used by runtime and precompute."""
    fair_esm_version = "unknown"

    if ESM2_MANIFEST.exists():
        try:
            manifest = json.loads(
                ESM2_MANIFEST.read_text(encoding="utf-8")
            )
...
def esm2_checkpoint_report() -> Dict[str, object]:
    ...
    if len(signatures) != len(manifest["files"]):
        missing = next(
            item["filename"]
            for item in manifest["files"]
            if not (directory / item["filename"]).is_file()
        )
        return {"ready": False, "reason": f"missing {missing}"}

    return _verify_esm2_files(
...
def _verify_esm2_files(
    ...
    for item in manifest["files"]:
        ...
        if digest != item["sha256"]:
            return {
                "ready": False,
                "reason": f"checksum mismatch for {item['filename']}",
            }
...
class KaptiveRunner:
    ...
    def type_assembly(self, fasta: str) -> KaptiveResult:
        ...
        with tempfile.TemporaryDirectory(prefix="phagex-kaptive-") as directory:
            ...
            locus = (
                record.get("Best match locus")
                or record.get("Best match locus name")
                or "Unknown"
            )
            confidence = (
                record.get("Match confidence")
                or record.get("Confidence")
                or "Unknown"
            )

            return KaptiveResult(
                best_match_locus=locus,
    ...
    def type_and_extract_assembly(self, fasta: str) -> IsolateLocusProteinSet:
        ...
        with tempfile.TemporaryDirectory(prefix="phagex-kaptive-") as directory:
            ...
            try:
                ...
                raise RuntimeError(
                    "Kaptive assembly typing timed out"
                ) from error
            except subprocess.CalledProcessError as error:
                detail = (
                    error.stderr
                    or "Kaptive assembly typing failed"
                ).strip().splitlines()[-1]
            ...
            lines = [
                line
                for line in json_output.read_text(
                    encoding="utf-8"
                ).splitlines()
                if line.strip()
            ]
            ...
            if len(lines) != 1:
                raise RuntimeError(
                    "Kaptive did not return exactly one isolate record"
                )

            result = isolate_proteins_from_kaptive_record(
                json.loads(lines[0])
            )
    ...
    def extract_reference_proteins(
        ...
        if not re.fullmatch(r"KL[0-9]{1,4}", locus):
            raise ValueError(
                "K locus must use the form KL followed by 1–4 digits"
            )
        ...
        if not sequences:
            raise ValueError(
                f"No reference proteins found for {locus}"
            )
        ...
        try:
            ...
            database_path = (
                Path(kaptive.__file__).parent.parent
                / "reference_database"
        ...
        return LocusProteinSet(
            ...
            names=names,
            sequences=sequences,
            protein_set_sha256=hashlib.sha256(
                normalized.encode()
            ).hexdigest(),
...
def parse_protein_fasta(value: str) -> tuple[List[str], List[str]]:
    ...
    for raw_line in value.splitlines():
        ...
        if line.startswith(">"):
            ...
            raise ValueError(
                "Protein FASTA must begin with a header"
            )
    ...
    if len(names) != len(sequences) or any(
        not sequence for sequence in sequences
    ):
        raise ValueError(
            "Protein FASTA contains an empty or malformed record"
        )
    ...
    if any(
        not allowed.fullmatch(sequence)
        for sequence in sequences
    ):
        raise ValueError(
            "Protein FASTA contains unsupported residues"
        )

    return names, sequences


def isolate_proteins_from_kaptive_record(
    ...
    if not re.fullmatch(r"KL[0-9]{1,4}", locus):
        raise ValueError(
            "Kaptive did not produce a valid Klebsiella K-locus call"
        )
    ...
    if not isinstance(genes, list) or not genes:
        raise ValueError(
            "Kaptive returned no expected genes inside the called locus"
        )
    ...
    if not isinstance(missing, list):
        raise ValueError(
            "Kaptive returned malformed missing-gene evidence"
        )
    ...
    if confidence != "Typeable":
        raise ValueError(
            f"Kaptive confidence is {confidence}; "
            "a Typeable call is required"
        )

    if missing:
        raise ValueError(
            "Missing K-locus genes prevent embedding: "
            f"{', '.join(map(str, missing[:5]))}"
        )

    if problems:
        raise ValueError(
            f"Kaptive reported locus problems ({problems}); "
    ...
    for gene in genes:
        if not isinstance(gene, dict):
            raise ValueError(
                "Kaptive returned a malformed gene record"
            )

        name = str(gene.get("gene", ""))
        sequence = (
            str(gene.get("protein_seq", ""))
            .rstrip("*")
            .upper()
        )

        partial = (
            str(gene.get("partial", "False")).lower()
            == "true"
        )
    ...
    if incomplete:
        raise ValueError(
            "Incomplete K-locus genes prevent embedding: "
            f"{', '.join(incomplete[:5])}"
        )

    if len(names) != len(set(names)):
        raise ValueError(
            "Kaptive returned duplicate expected genes"
        )
    ...
    return {
        ...
        "confidence": confidence,
        "percent_identity": float(
            record.get("percent_identity", 0)
        ),
        "percent_coverage": float(
            record.get("percent_coverage", 0)
        ),
        "names": names,
        "sequences": sequences,
        "protein_set_sha256": hashlib.sha256(
...
class EmbeddingCache:
    ...
    def key(
        ...
        """Build a deterministic identity for an ESM-2 representation.

        The final host representation is the mean of the protein-level
        representations, so protein order does not affect the vector.
        The exact protein sequence content is therefore part of the cache
        identity, along with the model and representation configuration.
        """
    ...
    def get(
        ...
        if vector.shape != (ESM2_DIMENSIONS,):
            return None
    ...
    def put(
        ...
        if value.shape != (ESM2_DIMENSIONS,):
            raise ValueError(
                f"Cannot cache embedding with shape {value.shape}; "
                f"expected ({ESM2_DIMENSIONS},)"
            )

        if not np.isfinite(value).all():
            raise ValueError(
                "Cannot cache non-finite embedding values"
            )
...
class ESM2Embedder:
    """Lazy local ESM-2 provider using a reproducible 1,280D contract."""
    ...
    def embed(
        ...
        if not proteins:
            raise ValueError(
                "At least one protein sequence is required"
            )
        ...
        if any(not sequence for sequence in cleaned):
            raise ValueError(
                "Protein sequences must not be empty"
            )

        if any(len(sequence) > 1022 for sequence in cleaned):
            raise ValueError(
                "ESM-2 provider limits individual proteins "
                "to 1,022 residues"
            )
        ...
        try:
            ...
            raise RuntimeError(
                "Local PyTorch and fair-esm are required "
                "for uncached ESM-2 inference"
            ) from error
        ...
        if self._model is None:
            ...
            if not checkpoint["ready"]:
                raise RuntimeError(
                    "Verified local ESM-2 checkpoint required: "
                    f"{checkpoint['reason']}"
                )

            model_path = (
                esm2_checkpoint_directory()
                / f"{ESM2_MODEL}.pt"
            )
        ...
        with torch.no_grad():
            output = self._model(
                tokens,
                repr_layers=[ESM2_LAYER],
                return_contacts=False,
            )["representations"][ESM2_LAYER]
        ...
        if combined.shape != (ESM2_DIMENSIONS,):
            raise ValueError(
                f"ESM-2 provider returned {combined.shape}, "
                f"expected ({ESM2_DIMENSIONS},)"
            )

        if not np.isfinite(combined).all():
            raise ValueError(
                "ESM-2 provider returned non-finite values"
            )
...
class ReferenceLocusFeaturePipeline:
    """Extract canonical K-locus proteins and pass them to local ESM-2.

    This is a reference-locus feature path, not proof that an uploaded
    isolate has a complete or identical locus. Isolate-specific CDS
    extraction remains gated.
    """

    def __init__(
        self,
        extractor: KaptiveRunner,
    ...
    def build(
        ...
        proteins = self.extractor.extract_reference_proteins(
            locus
        )

        embedding = np.asarray(
            self.embedder.embed(proteins.sequences),
            dtype=np.float32,
        )

        if embedding.shape != (ESM2_DIMENSIONS,):
            raise ValueError(
                f"ESM-2 provider returned {embedding.shape}, "
                f"expected ({ESM2_DIMENSIONS},)"
            )
        ...
        return LocusFeatureResult(
            ...
            embedding_cache_key=EmbeddingCache.key(
                proteins.sequences
            ),
            embedding=embedding,
        )
...
class IsolateLocusFeaturePipeline:
    ...
    def build(
        ...
        proteins = self.extractor.type_and_extract_assembly(
            fasta
        )
