...
**Status:** A calibrated XGBoost classifier is trained on 10,006 real labeled pairs with host-disjoint testing, permutation importance, explicit abstention, five repeated holdouts, host-level bootstrap confidence intervals, ten-bin calibration reporting, and threshold error counts. Release remains explicitly blocked pending a second external dataset, biologically meaningful subgroup labels, explanation review, and independent review.
